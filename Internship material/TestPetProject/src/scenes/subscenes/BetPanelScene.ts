...
import { BetOptionsControl } from "app/controls/BetOptionsControl";
import BetSelectorBtnsControl from "app/controls/button/BetSelectorBtnsControl";
import FreeSpinCounter from "app/controls/ui/FreeSpinCounter";
...
export default class BetPanelScene extends BaseScene {
    ...
    private freeSpinsCounter: FreeSpinCounter = new FreeSpinCounter();
    ...
    private plank: SpriteControl = new SpriteControl("bet_panel_blank.png");
    ...
    async compose() {
        ...
        const betButton = this.betSelectorControl;
        ...
        betButton.betButton.addExtension(new ControlShowerExtension(this.betControl));
        ...
        gameModel.game.signals.freeSpinsCounter.show.add(this.onShowFreeSpinsCounter, this);
        gameModel.game.signals.freeSpinsCounter.hide.add(this.onHideFreeSpinsCounter, this);
    ...
    private onSpinRemainsChanged(data: {spinRemains: number, generalSpins: number}) {
        this.freeSpinsCounter.setSpinsNumberAndGeneral(data.spinRemains, data.generalSpins);
    }

    private onUpdateSpinBtnCounter(spinsRemains: number) {
        this.freeSpinsCounter.setSpinsNumber(spinsRemains);
    }
    ...
    private onShowFreeSpinsCounter() {
        this.freeSpinsCounter.showCounter();
    }

    private onHideFreeSpinsCounter() {
        this.freeSpinsCounter.hideCounter();
    }
}
