...
export type TReelInfo = {
    ...
    symbol: { width: number, height: number, gap: number, scale: number, offsetX:number }
};
type TAutoplayInfo = {
    spins: number
};
    ...
    constructor() {
        ...
        this.ready = new Promise<void>(async resolve => {
            ...
            this.mainGameInfo = Object.assign(response,
                <TReelInfo>{
                    reels: {amount: 5, height: 3, reelGap: 50, reelFrameGap: 18},
                    symbol: {width: 240, height: 275, gap: 130, scale: 1.100, offsetX:15},
                },
                <TTimings>{
                    timings: {
            ...
            console.log("mainGameInfo:", this.mainGameInfo);
            resolve();
        });
