...
export default class FreeSpinModel {
    ...
    totalWin = 0;
    generalSpins = 0;
    readonly freeSpinsAmountChanged = new Signal<{spinRemains: number, generalSpins: number }>();


    setup(freeSpins: TSpinResult[], initialTotalWin: number) {
        this.freeSpins = freeSpins;
        this.generalSpins = freeSpins.length;
        this.spinRemains = freeSpins.length;
        this.totalWin += initialTotalWin;
        this.freeSpinsAmountChanged.emit({spinRemains: this.spinRemains, generalSpins: this.generalSpins});
    }
    ...
    addSpins(freeSpinFeature: TFreeSpinsFeatureResult, toTheEnd = false) {
        ...
        this.spinRemains = this.freeSpins.length - this.spinIndex - 1;
        this.generalSpins += freeSpins.spins.length;
        this.freeSpinsAmountChanged.emit({spinRemains: this.spinRemains, generalSpins: this.generalSpins});
    }
}
