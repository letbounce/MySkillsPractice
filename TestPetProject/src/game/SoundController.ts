...
class ReelSoundsController implements Composer, Disposer {
  ...
  compose(): void {
    ...
    gameSignals.reels.scatterReels.add(
      (scatterReels) => (this.scatterReels = scatterReels),
      this
    );
  }
  ...
  private onAnticipationShown() {
    this.howler.fade(0.125, 0, 2000, this.anticipationSoundId);
    this.anticipationSoundId = this.howler.play("anticipation");
    this.howler.fade(0, 0.125, 100, this.anticipationSoundId);
  }
}
...
class BackgroundSoundController implements Composer, Disposer {
  ...
  private state: "free_spins" | "free_game" | "base_game" | "auto_spins_game" =
    "base_game";
  ...
  private onFreeSpinsIntro(_: number, resolve?: () => void) {
    ...
    this.themeSoundId = this.howler.play("free-spin-pop-up");
...
export default class SoundController implements Composer, Disposer {
  ...
  constructor(private howler: Howl) {
    ...
    this.backgroundSoundController = new BackgroundSoundController(
      howler,
      this.coinShower
    );
  ...
  compose(): void {
    ...
    gameSignals.ui.options.toggleSound.add(() => {
      ...
      gameLocalStorage.setItem(
        "isSoundMuted",
        gameModel.isSoundMuted.toString()
      );
    }, this);
...
class SoundCoinShower {
  ...
  play() {
    ...
    this.intervalId = setInterval(() => {
      const id = this.howl.play("cash-symbol");
